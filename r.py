...
while count not in [0]:
    ...
    for  video_link in playlist :
        ...
        if temm == count:
            try:
                sm = YouTube(video_link , on_progress_callback=on_progress).streams.filter(res="720p").first().download(output_path="E:\Videos\React Hooks", filename_prefix=str(count) +" ")

...
playlist = Playlist(val).video_urls
count =0
for video_link in playlist:
    count=count+1
    if count > 0: 
        try:
            sm = YouTube(video_link , on_progress_callback=on_progress).streams.filter(res="720p").first().download(output_path="./", filename_prefix=str(count) +" ")
